...
- `manifest.json`：`host_permissions` 含 `https://api.shopradar.uk/*`
- 抓取店铺仍需 `<all_urls>`（访问各 Shopify 站）

本地调试：复制 `extension-config.dev.example.js` 为 `extension-config.local.js`，`debug: true`，`apiBase: http://localhost:3000`，并在 `popup.html` 临时改为引入 `extension-config.local.js`（**不要打进商店 zip**）。
