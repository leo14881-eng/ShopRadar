...
const FAMOUS_STORES_CACHE_KEY = 'shopradar:trending:famous-stores:v1';
...
async function getFamousStoresCache() {
  const redis = assertClientReady();
  try {
    const raw = await redis.get(FAMOUS_STORES_CACHE_KEY);
    if (!raw) {
      return null;
    }
    return JSON.parse(raw);
...
async function setFamousStoresCache(payload) {
  const redis = assertClientReady();
  try {
    await redis.set(FAMOUS_STORES_CACHE_KEY, JSON.stringify(payload), {
      EX: TRENDING_CACHE_TTL_SEC,
    });
    return true;
  } catch (writeErr) {
...
async function invalidateFamousStoresCache() {
  if (!client || !client.isOpen) {
    return false;
  }
  try {
    await client.del(FAMOUS_STORES_CACHE_KEY);
    return true;
  } catch (delErr) {
    const err = new Error('Redis DEL 失败: ' + delErr.message);
    err.code = 'REDIS_DEL_FAILED';
...
module.exports = {
  ...
  FAMOUS_STORES_CACHE_KEY: FAMOUS_STORES_CACHE_KEY,
  ...
  getFamousStoresCache: getFamousStoresCache,
  setFamousStoresCache: setFamousStoresCache,
  invalidateFamousStoresCache: invalidateFamousStoresCache,
};
